...
metadata:
  ...
  source: fleet:commands/review.md
---
...
Run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` from the repo root to discover `FEATURE_DIR`. Then read these artifacts:
