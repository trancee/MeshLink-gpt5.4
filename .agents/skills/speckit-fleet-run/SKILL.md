...
metadata:
  ...
  source: fleet:commands/fleet.md
---
...
Run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` from the repo root to get the feature directory paths as JSON. Parse the output to get `FEATURE_DIR`.
