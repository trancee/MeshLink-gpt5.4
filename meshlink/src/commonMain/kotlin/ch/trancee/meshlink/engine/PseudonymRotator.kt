...
public class PseudonymRotator(
    ...
    public fun staggerMillis(nodeId: ByteArray): Long {
        require(nodeId.isNotEmpty()) {
            "PseudonymRotator nodeId must not be empty."
        }
        if (maxStaggerMillis == 0L) {
            return 0L
        }

        val digest: ByteArray = cryptoProvider.hmacSha256(
            key = nodeId,
            message = STAGGER_CONTEXT,
        )
        val accumulator: Long = digest.take(n = 8).fold(initial = 0L) { value, byte ->
            (value shl 8) or (byte.toLong() and 0xFFL)
        }
        return accumulator % (maxStaggerMillis + 1)
    }

    public fun pseudonymForEpoch(
        identityKey: ByteArray,
        epoch: Long,
    ): ByteArray {
        require(identityKey.isNotEmpty()) {
            "PseudonymRotator identityKey must not be empty."
        }
        require(epoch >= 0) {
            "PseudonymRotator epoch must be greater than or equal to 0."
        }

        val epochMessage: ByteArray = byteArrayOf(
        ...
        return cryptoProvider.hmacSha256(
            key = identityKey,
