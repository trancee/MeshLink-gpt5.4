...
public class InboundValidator(
    ...
    public fun validate(encoded: ByteArray): ValidationResult {
        ...
        return when (type) {
            ...
            MessageType.UPDATE -> validateFixedPayload(
                payloadSize = payloadSize,
                expectedPayloadSize = UpdateMessageCodec.DESTINATION_PEER_ID_SIZE + Int.SIZE_BYTES + Int.SIZE_BYTES,
                failureCode = ValidationFailureCode.UPDATE_PAYLOAD_SIZE_INVALID,
                failureReason = "UPDATE payload must be exactly ${UpdateMessageCodec.DESTINATION_PEER_ID_SIZE + Int.SIZE_BYTES + Int.SIZE_BYTES} bytes.",
            )
            MessageType.CHUNK -> validateChunk(payloadSize = payloadSize)
            MessageType.CHUNK_ACK -> validateChunkAck(payloadSize = payloadSize)
            MessageType.DELIVERY_ACK -> validateFixedPayload(
                payloadSize = payloadSize,
            ...
            MessageType.NACK -> validateFixedPayload(
                payloadSize = payloadSize,
            ...
            MessageType.KEEPALIVE -> validateFixedPayload(
                payloadSize = payloadSize,
            ...
            MessageType.RESUME_REQUEST -> validateFixedPayload(
                payloadSize = payloadSize,
                expectedPayloadSize = Long.SIZE_BYTES + Long.SIZE_BYTES,
                failureCode = ValidationFailureCode.RESUME_REQUEST_PAYLOAD_SIZE_INVALID,
                failureReason = "RESUME_REQUEST payload must be exactly ${Long.SIZE_BYTES + Long.SIZE_BYTES} bytes.",
            )
            MessageType.ROTATION_ANNOUNCEMENT -> validateFixedPayload(
                payloadSize = payloadSize,
                expectedPayloadSize = RotationAnnouncementMessageCodec.PUBLIC_KEY_SIZE + RotationAnnouncementMessageCodec.PUBLIC_KEY_SIZE + RotationAnnouncementMessageCodec.SIGNATURE_SIZE,
                failureCode = ValidationFailureCode.ROTATION_ANNOUNCEMENT_PAYLOAD_SIZE_INVALID,
                failureReason = "ROTATION_ANNOUNCEMENT payload must be exactly ${RotationAnnouncementMessageCodec.PUBLIC_KEY_SIZE + RotationAnnouncementMessageCodec.PUBLIC_KEY_SIZE + RotationAnnouncementMessageCodec.SIGNATURE_SIZE} bytes.",
            )
        }
    }
    ...
    private fun validateBroadcast(encoded: ByteArray, payloadSize: Int): ValidationResult {
        ...
        val maxHops: UByte = encoded[payloadOffset + HelloMessageCodec.PEER_ID_SIZE + Int.SIZE_BYTES].toUByte()
    ...
    public companion object {
        ...
        private const val BROADCAST_HEADER_SIZE: Int = BroadcastMessageCodec.ORIGIN_PEER_ID_SIZE + Int.SIZE_BYTES + 1
