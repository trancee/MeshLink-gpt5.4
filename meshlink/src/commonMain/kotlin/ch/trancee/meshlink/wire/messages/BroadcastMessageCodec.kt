...
public object BroadcastMessageCodec {
    ...
    public fun encode(message: BroadcastMessage): ByteArray {
        if (message.originPeerId.size != ORIGIN_PEER_ID_SIZE) {
            throw IllegalArgumentException("Broadcast originPeerId must be exactly $ORIGIN_PEER_ID_SIZE bytes.")
        }
    ...
    public fun decode(payload: ByteArray): BroadcastMessage {
        if (payload.size < HEADER_SIZE) {
            throw IllegalArgumentException("BROADCAST payload must contain originPeerId, sequenceNumber, and maxHops.")
        }
