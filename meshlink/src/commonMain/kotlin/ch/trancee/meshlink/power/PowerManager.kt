...
public class PowerManager(
    ...
    public fun evaluate(connections: List<ManagedConnection>): PowerDecision {
        ...
        val activeTier: PowerTier = modeEngine.evaluate(
            currentTier = currentTier,
            batteryPercent = batteryPercent,
            config = config,
        )
        currentTier = activeTier
        val profile: PowerProfile = BleConnectionParameterPolicy.profileFor(tier = activeTier, config = config)
        val canAcceptConnection: Boolean = ConnectionLimiter.canAcceptConnection(
            currentConnectionCount = connections.size,
            tier = activeTier,
            config = config,
        )
        val shedConnections: List<ManagedConnection> = TieredShedder.shed(
            connections = connections,
            targetConnectionCount = profile.maxConnections,
        )
