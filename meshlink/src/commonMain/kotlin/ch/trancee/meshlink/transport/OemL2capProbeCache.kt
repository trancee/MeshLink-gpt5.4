...
public class OemL2capProbeCache(
    ...
    init {
        require(maxEntries > 0) {
            "OemL2capProbeCache maxEntries must be greater than 0."
        }
    }

    public fun get(deviceModel: String): Boolean? {
        val normalizedDeviceModel: String = deviceModel.normalizeDeviceModel()
        val existingIndex: Int = entries.indexOfFirst { entry -> entry.deviceModel == normalizedDeviceModel }
        if (existingIndex < 0) {
            return null
        }
    ...
    public fun record(
        ...
        entries += OemL2capProbeCacheEntry(
...
private fun String.normalizeDeviceModel(): String {
    ...
    require(normalizedValue.isNotEmpty()) {
        "OemL2capProbeCache deviceModel must not be blank."
    }
