...
public class L2capRetryScheduler(
    ...
    public fun delayMillisForAttempt(attempt: Int): Long {
        require(attempt >= 0) {
            "L2capRetryScheduler attempt must be greater than or equal to 0."
        }

        var baseDelayMillis: Long = initialDelayMillis
        repeat(times = attempt) {
            baseDelayMillis = when {
        ...
        return (baseDelayMillis + jitterOffsetMillis).coerceIn(minimumValue = 0L, maximumValue = maxDelayMillis)
    }
