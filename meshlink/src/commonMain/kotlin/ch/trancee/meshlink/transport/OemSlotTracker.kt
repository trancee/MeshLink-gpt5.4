...
public class OemSlotTracker(
    ...
    init {
        require(maxSlots > 0) {
            "OemSlotTracker maxSlots must be greater than 0."
        }
    }

    public fun acquire(ownerId: String): Int? {
        val normalizedOwnerId: String = ownerId.normalizeOwnerId()
        allocations.firstOrNull { allocation -> allocation.ownerId == normalizedOwnerId }?.let { allocation ->
            return allocation.slot
        }

        val occupiedSlots: Set<Int> = allocations.map { allocation -> allocation.slot }.toSet()
        val availableSlot: Int = (0 until maxSlots).firstOrNull { slot -> slot !in occupiedSlots } ?: return null
        allocations += OemSlotAllocation(
    ...
    public fun release(ownerId: String): Unit {
        val normalizedOwnerId: String = ownerId.normalizeOwnerId()
        val existingIndex: Int = allocations.indexOfFirst { allocation -> allocation.ownerId == normalizedOwnerId }
        if (existingIndex >= 0) {
...
private data class OemSlotAllocation(
    val ownerId: String,
    val slot: Int,
)
...
private fun String.normalizeOwnerId(): String {
    ...
    require(normalizedValue.isNotEmpty()) {
        "OemSlotTracker ownerId must not be blank."
    }
