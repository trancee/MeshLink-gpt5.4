...
public class L2capFrameCodec(
    ...
    public fun append(chunk: ByteArray): List<ByteArray> {
        ...
        while (bufferedBytes.size >= Int.SIZE_BYTES) {
            val length: Int = ReadBuffer(source = bufferedBytes.copyOfRange(fromIndex = 0, toIndex = Int.SIZE_BYTES)).readInt()
            ...
            bufferedBytes = bufferedBytes.copyOfRange(fromIndex = totalFrameBytes, toIndex = bufferedBytes.size)
        }
