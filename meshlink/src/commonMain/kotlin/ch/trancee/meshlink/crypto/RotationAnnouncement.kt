...
public data class RotationAnnouncement(
    ...
    public fun verify(provider: CryptoProvider): Boolean {
        ...
        return provider.ed25519Verify(
            ...
            message = signingPayload(previousPublicKey = previousPublicKey, nextPublicKey = nextPublicKey),
            signature = signature,
        )
    }
    ...
    public companion object {
        ...
        private fun validatePublicKey(
            ...
            if (value.size != Identity.PUBLIC_KEY_SIZE) {
                throw IllegalArgumentException("RotationAnnouncement $name must be exactly ${Identity.PUBLIC_KEY_SIZE} bytes.")
            }
        }

        private fun validateSecretKey(secretKey: ByteArray): Unit {
            if (secretKey.size != Identity.SECRET_KEY_SIZE) {
                throw IllegalArgumentException("RotationAnnouncement previous secretKey must be exactly ${Identity.SECRET_KEY_SIZE} bytes.")
            }
        }

        private fun validateSignature(signature: ByteArray): Unit {
            if (signature.size != SIGNATURE_SIZE) {
                throw IllegalArgumentException("RotationAnnouncement signature must be exactly $SIGNATURE_SIZE bytes.")
            }
        }

        private fun signingPayload(
            previousPublicKey: ByteArray,
            nextPublicKey: ByteArray,
        ): ByteArray {
