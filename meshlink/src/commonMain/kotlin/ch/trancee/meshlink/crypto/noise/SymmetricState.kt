...
public class SymmetricState(
    ...
    public fun mixHash(data: ByteArray): ByteArray {
        handshakeHash = provider.hmacSha256(
            key = handshakeHash,
            message = data,
        )
    ...
    public fun mixKey(inputKeyMaterial: ByteArray): ByteArray {
        val derivedKeyMaterial: ByteArray = provider.hkdfSha256(
            ikm = inputKeyMaterial,
            salt = chainingKey,
            info = KEY_DERIVATION_INFO,
            outputLength = DERIVED_KEY_MATERIAL_SIZE,
        )
        chainingKey = derivedKeyMaterial.copyOfRange(
            fromIndex = 0,
            toIndex = KEY_SIZE,
        )
        val newCipherKey: ByteArray = derivedKeyMaterial.copyOfRange(
            fromIndex = KEY_SIZE,
            toIndex = DERIVED_KEY_MATERIAL_SIZE,
        )
    ...
    public fun encryptAndHash(plaintext: ByteArray): ByteArray {
        val ciphertext: ByteArray = if (cipherKey == null) {
            plaintext.copyOf()
        } else {
    ...
    public fun decryptAndHash(ciphertext: ByteArray): ByteArray {
        val plaintext: ByteArray = if (cipherKey == null) {
            ciphertext.copyOf()
        } else {
    ...
    public companion object {
        ...
        private val KEY_DERIVATION_INFO: ByteArray = "meshlink-noise-symmetric-state".encodeToByteArray()
