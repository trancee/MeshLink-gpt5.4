...
public class PresenceTracker(
    ...
    init {
        require(timeoutMillis > 0) {
            "PresenceTracker timeoutMillis must be greater than 0."
        }
    }
    ...
    public fun sweep(nowEpochMillis: Long): List<PresenceEvent> {
        ...
        val disappearedPeers: List<PeerIdHex> = peersById.entries
            .filter { entry -> nowEpochMillis - entry.value >= timeoutMillis }
            .map { entry -> PeerIdHex(value = entry.key) }

        disappearedPeers.forEach { peerId ->
            peersById.remove(peerId.value)
        }
