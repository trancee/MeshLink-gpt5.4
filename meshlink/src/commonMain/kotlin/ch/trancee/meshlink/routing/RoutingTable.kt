...
public data class RouteEntry(
    ...
    init {
        require(metric >= 0) {
            "RouteEntry metric must be greater than or equal to 0."
        }
        require(sequenceNumber >= 0) {
            "RouteEntry sequenceNumber must be greater than or equal to 0."
        }
...
public class RoutingTable {
    ...
    public fun install(route: RouteEntry): Unit {
        val routesForDestination: MutableList<RouteEntry> = routesByDestination
            .getOrPut(route.destinationPeerId.value) { mutableListOf() }

        val existingIndex: Int = routesForDestination.indexOfFirst { existingRoute ->
    ...
    public fun bestRoute(destinationPeerId: PeerIdHex): RouteEntry? {
        val routesForDestination: List<RouteEntry> = routesByDestination[destinationPeerId.value] ?: return null
    ...
    public fun routesFor(destinationPeerId: PeerIdHex): List<RouteEntry> {
        val routesForDestination: List<RouteEntry> = routesByDestination[destinationPeerId.value] ?: return emptyList()
        return routesForDestination.sortedWith(routeOrdering)
    }

    public fun withdraw(
        destinationPeerId: PeerIdHex,
        nextHopPeerId: PeerIdHex,
    ): Unit {
        val routesForDestination: MutableList<RouteEntry> = routesByDestination[destinationPeerId.value] ?: return
    ...
    public companion object {
        private val routeOrdering: Comparator<RouteEntry> = compareBy<RouteEntry> { route -> route.metric }
