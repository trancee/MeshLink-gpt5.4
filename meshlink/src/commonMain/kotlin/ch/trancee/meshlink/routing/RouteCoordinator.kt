...
public class RouteCoordinator(
    ...
    public fun isFeasible(
        ...
        require(metric >= 0) {
            "RouteCoordinator metric must be greater than or equal to 0."
        }
    ...
    public fun recordAcceptedRoute(
        ...
        require(isFeasible(destinationPeerId = destinationPeerId, sequenceNumber = sequenceNumber, metric = metric)) {
            "RouteCoordinator rejected infeasible route for ${destinationPeerId.value}."
        }

        sourceRecords[destinationPeerId.value] = SourceRecord(
...
public data class SourceRecord(
    ...
    init {
        ...
        require(metric >= 0) {
            "SourceRecord metric must be greater than or equal to 0."
        }
    }
}
