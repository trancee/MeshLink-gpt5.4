...
public class DedupSet(
  ...
  init {
    require(maxEntries > 0) { "DedupSet maxEntries must be greater than 0." }
  ...
  public fun isDuplicate(key: ByteArray, nowEpochMillis: Long): Boolean {
    ...
    entries += DedupEntry(key = normalizedKey, lastSeenEpochMillis = nowEpochMillis)
    return false
  }
...
private data class DedupEntry(val key: String, val lastSeenEpochMillis: Long)
