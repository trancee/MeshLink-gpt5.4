...
public class TransferSessionTest {
    ...
    public fun invalidInputs_areRejected(): Unit {
        ...
        // Act
        val idError = assertFailsWith<IllegalArgumentException> {
            TransferSession(
                transferId = "   ",
        ...
        val windowSizeError = assertFailsWith<IllegalArgumentException> {
            session.nextChunks(windowSize = 0)
        }

        // Assert
        assertEquals(expected = "TransferSession transferId must not be blank.", actual = idError.message)
        assertEquals(expected = "TransferSession payload must not be empty.", actual = payloadError.message)
        assertEquals(expected = "TransferSession chunkSizeBytes must be greater than 0.", actual = chunkSizeError.message)
        assertEquals(expected = "TransferSession windowSize must be greater than 0.", actual = windowSizeError.message)
    }
}
