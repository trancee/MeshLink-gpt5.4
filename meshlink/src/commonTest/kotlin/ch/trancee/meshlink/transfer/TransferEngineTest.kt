...
public class TransferEngineTest {
    ...
    public fun startTransfer_schedulesAndChunksPayloadsAccordingToTheSelectedPolicy(): Unit {
        ...
        // Act
        val started = engine.startTransfer(
            transferId = "tx-1",
    ...
    public fun acknowledge_retransmitsOnlyMissingChunksAndCompletesTheTransfer(): Unit {
        ...
        // Act
        val firstProgress = engine.acknowledge(transferId = "tx-1", chunkIndex = 0, nowEpochMillis = 10L)
        val secondProgress = engine.acknowledge(transferId = "tx-1", chunkIndex = 2, nowEpochMillis = 30L)
        ...
        // Assert
        assertEquals(
            expected = TransferEvent.Progress(transferId = "tx-1", acknowledgedBytes = 1L, totalBytes = 3L),
            actual = firstProgress,
        )
        assertEquals(
            expected = TransferEvent.Progress(transferId = "tx-1", acknowledgedBytes = 2L, totalBytes = 3L),
    ...
    public fun cancelAndTimeout_releaseTransfersAndReportFailures(): Unit {
        ...
        assertEquals(
            expected = listOf(TransferEvent.Failed(transferId = "timeout-me", reason = FailureReason.TIMEOUT)),
            actual = timedOut,
        )
    ...
    public fun receiveChunk_reassemblesInboundTransfersOnceAllChunksArrive(): Unit {
        ...
        // Act
        val first = engine.receiveChunk(transferId = "rx-1", chunkIndex = 0, totalChunks = 2, payload = byteArrayOf(0x01, 0x02))
        val second = engine.receiveChunk(transferId = "rx-1", chunkIndex = 1, totalChunks = 2, payload = byteArrayOf(0x03))
    ...
    public fun invalidInputs_andMissingTransfers_areRejected(): Unit {
        ...
        // Act
        val duplicateTransferError = assertFailsWith<IllegalArgumentException> {
            engine.startTransfer(
                transferId = "tx-1",
                recipientPeerId = PeerIdHex(value = "00112233"),
                priority = Priority.NORMAL,
        ...
        val missingAcknowledge = engine.acknowledge(transferId = "missing", chunkIndex = 0, nowEpochMillis = 0L)
        val missingDelay = engine.recommendedDelayMillis(transferId = "missing")
        val negativeTimestampError = assertFailsWith<IllegalArgumentException> {
            engine.startTransfer(
                transferId = "tx-2",
                recipientPeerId = PeerIdHex(value = "00112233"),
                priority = Priority.NORMAL,
        ...
        val missingTransferError = assertFailsWith<IllegalArgumentException> {
            engine.nextChunks(transferId = "missing")
        }
        val negativeAcknowledgeTimestampError = assertFailsWith<IllegalArgumentException> {
            engine.acknowledge(transferId = "tx-1", chunkIndex = 0, nowEpochMillis = -1L)
        }
        val negativeTimeoutTimestampError = assertFailsWith<IllegalArgumentException> {
            engine.failTimedOut(nowEpochMillis = -1L)
        }
        val invalidReceiveChunkIndexError = assertFailsWith<IllegalArgumentException> {
            engine.receiveChunk(transferId = "rx", chunkIndex = 2, totalChunks = 2, payload = byteArrayOf(0x01))
        }
        val negativeReceiveChunkIndexError = assertFailsWith<IllegalArgumentException> {
            engine.receiveChunk(transferId = "rx", chunkIndex = -1, totalChunks = 2, payload = byteArrayOf(0x01))
        }
        val invalidReceiveChunkCountError = assertFailsWith<IllegalArgumentException> {
            engine.receiveChunk(transferId = "rx", chunkIndex = 0, totalChunks = 0, payload = byteArrayOf(0x01))
        }

        // Assert
        assertEquals(expected = "TransferEngine already has an active transfer for tx-1.", actual = duplicateTransferError.message)
        assertEquals(expected = null, actual = missingAcknowledge)
        assertEquals(expected = null, actual = missingDelay)
        assertEquals(expected = "TransferEngine nowEpochMillis must be greater than or equal to 0.", actual = negativeTimestampError.message)
        assertEquals(expected = "TransferEngine has no active transfer for missing.", actual = missingTransferError.message)
        assertEquals(expected = "TransferEngine nowEpochMillis must be greater than or equal to 0.", actual = negativeAcknowledgeTimestampError.message)
        assertEquals(expected = "TransferEngine nowEpochMillis must be greater than or equal to 0.", actual = negativeTimeoutTimestampError.message)
        assertEquals(expected = "TransferEngine chunkIndex must be between 0 and 1.", actual = invalidReceiveChunkIndexError.message)
        assertEquals(expected = "TransferEngine chunkIndex must be between 0 and 1.", actual = negativeReceiveChunkIndexError.message)
        assertEquals(expected = "TransferEngine totalChunks must be greater than 0.", actual = invalidReceiveChunkCountError.message)
    }
}
