...
public class TransferFoundationTest {
  ...
  public fun transferConfig_rejectsInvalidValues(): Unit {
    ...
    // Assert
    assertEquals(
      expected = "TransferConfig timeoutMillis must be greater than 0.",
      actual = timeoutError.message,
    )
    assertEquals(
      expected = "TransferConfig retransmitLimit must be greater than or equal to 0.",
      actual = retryError.message,
    )
    assertEquals(
      expected = "TransferConfig windowSize must be greater than 0.",
  ...
  public fun chunkSizePolicy_rejectsInvalidChunkSizes(): Unit {
    ...
    // Assert
    assertEquals(
      expected = "ChunkSizePolicy gattChunkSizeBytes must be greater than 0.",
      actual = gattError.message,
    )
    assertEquals(
      expected = "ChunkSizePolicy l2capChunkSizeBytes must be greater than 0.",
      actual = l2capError.message,
    )
  }
  ...
  public fun transferEvents_andPriorityTypes_retainTheirConstructorValues(): Unit {
    // Arrange
    // Act
    val started: TransferEvent =
      TransferEvent.Started(transferId = "tx-1", priority = Priority.HIGH)
    val progress: TransferEvent =
      TransferEvent.Progress(transferId = "tx-1", acknowledgedBytes = 128L, totalBytes = 256L)
    val complete: TransferEvent = TransferEvent.Complete(transferId = "tx-1", totalBytes = 256L)
    val failed: TransferEvent =
      TransferEvent.Failed(transferId = "tx-1", reason = FailureReason.TIMEOUT)
    ...
    assertEquals(
      expected = TransferEvent.Complete(transferId = "tx-1", totalBytes = 256L),
      actual = complete,
    )
    assertEquals(
      expected = TransferEvent.Failed(transferId = "tx-1", reason = FailureReason.TIMEOUT),
      actual = failed,
    )
  }
}
