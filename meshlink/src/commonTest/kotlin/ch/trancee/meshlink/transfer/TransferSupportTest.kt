...
public class TransferSupportTest {
  ...
  public fun sackTracker_rejectsInvalidConstructionAndChunkIndices(): Unit {
    // Arrange
    // Act
    val initError = assertFailsWith<IllegalArgumentException> { SackTracker(totalChunks = 0) }
    val sackTracker = SackTracker(totalChunks = 2)
    val acknowledgeTooLargeError =
      assertFailsWith<IllegalArgumentException> { sackTracker.acknowledge(chunkIndex = 2) }
    val acknowledgeNegativeError =
      assertFailsWith<IllegalArgumentException> { sackTracker.acknowledge(chunkIndex = -1) }
    val isAcknowledgedNegativeError =
      assertFailsWith<IllegalArgumentException> { sackTracker.isAcknowledged(chunkIndex = -1) }
    val isAcknowledgedTooLargeError =
      assertFailsWith<IllegalArgumentException> { sackTracker.isAcknowledged(chunkIndex = 2) }

    // Assert
    assertEquals(
      expected = "SackTracker totalChunks must be greater than 0.",
      actual = initError.message,
    )
    assertEquals(
      expected = "SackTracker chunkIndex must be between 0 and 1.",
      actual = acknowledgeTooLargeError.message,
    )
    assertEquals(
      expected = "SackTracker chunkIndex must be between 0 and 1.",
      actual = acknowledgeNegativeError.message,
    )
    assertEquals(
      expected = "SackTracker chunkIndex must be between 0 and 1.",
      actual = isAcknowledgedNegativeError.message,
    )
    assertEquals(
      expected = "SackTracker chunkIndex must be between 0 and 1.",
      actual = isAcknowledgedTooLargeError.message,
    )
  }
  ...
  public fun resumeCalculator_usesTheHighestContiguousSackOffset(): Unit {
    ...
    // Act
    val actual =
      ResumeCalculator.resumeOffsetBytes(
  ...
  public fun resumeCalculator_rejectsInvalidChunkSizesAndTotalBytes(): Unit {
    ...
    // Assert
    assertEquals(
      expected = "ResumeCalculator chunkSizeBytes must be greater than 0.",
      actual = chunkSizeError.message,
    )
    assertEquals(
      expected = "ResumeCalculator totalBytes must be greater than or equal to 0.",
      actual = totalBytesError.message,
    )
  }
  ...
  public fun observationRateController_returnsZeroBeforeTwoSamplesAndRejectsInvalidInputs(): Unit {
    ...
    val initError =
      assertFailsWith<IllegalArgumentException> { ObservationRateController(windowSize = 0) }
    val timestampError =
      assertFailsWith<IllegalArgumentException> {
        controller.recordAcknowledgement(timestampMillis = -1L)
      }
    val decreasingTimestampError =
      assertFailsWith<IllegalArgumentException> {
    ...
    assertEquals(
      expected = "ObservationRateController windowSize must be greater than 0.",
      actual = initError.message,
    )
    assertEquals(
      expected = "ObservationRateController timestampMillis must be greater than or equal to 0.",
      actual = timestampError.message,
    )
    assertEquals(
      expected = "ObservationRateController timestamps must be non-decreasing.",
      actual = decreasingTimestampError.message,
    )
  }
  ...
  public fun transferScheduler_requeuesExistingTransfersAndRejectsBlankIds(): Unit {
    ...
    assertEquals(
      expected = "TransferScheduler transferId must not be blank.",
