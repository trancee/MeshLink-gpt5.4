...
public class UpdateMessageCodecTest {
    ...
    public fun encodeAndDecode_roundTripUpdateMessage(): Unit {
        // Arrange
        val expectedDestinationPeerId: ByteArray = byteArrayOf(0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C)
    ...
    public fun encode_throwsWhenDestinationPeerIdLengthIsInvalid(): Unit {
        // Arrange
        val message = UpdateMessage(
            destinationPeerId = byteArrayOf(0x01),
            metric = 1,
            seqno = 2,
        )
        ...
        val error = assertFailsWith<IllegalArgumentException> {
            UpdateMessageCodec.encode(message = message)
        }
        ...
        assertEquals(
            ...
            message = "UpdateMessageCodec should reject destination peer identifiers with the wrong length",
        )
    }
    ...
    public fun decode_throwsWhenPayloadLengthIsInvalid(): Unit {
        ...
        // Act
        val error = assertFailsWith<IllegalArgumentException> {
            UpdateMessageCodec.decode(payload = payload)
        }
