...
public class RoutedMessageCodecTest {
  ...
  public fun encodeAndDecode_roundTripRoutedMessage(): Unit {
    ...
    val message = RoutedMessage(hopCount = 1u, maxHops = 3u, payload = expectedPayload)
  ...
  public fun encode_writesHopHeaderBeforePayload(): Unit {
    // Arrange
    val message = RoutedMessage(hopCount = 2u, maxHops = 5u, payload = byteArrayOf(0x51, 0x52))
  ...
  public fun decode_throwsWhenPayloadDoesNotContainHopHeader(): Unit {
    ...
    // Act
    val error =
      assertFailsWith<IllegalArgumentException> { RoutedMessageCodec.decode(payload = payload) }
    ...
    assertEquals(
      ...
      message =
        "RoutedMessageCodec should reject payloads that do not contain both hop header bytes",
    )
  }
}
