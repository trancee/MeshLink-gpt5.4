...
public class HelloMessageCodecTest {
  ...
  public fun encode_throwsWhenPeerIdLengthIsInvalid(): Unit {
    // Arrange
    val message = HelloMessage(peerId = byteArrayOf(0x01, 0x02), appIdHash = 1)

    // Act
    val error =
      assertFailsWith<IllegalArgumentException> { HelloMessageCodec.encode(message = message) }
  ...
  public fun decode_throwsWhenPayloadLengthIsInvalid(): Unit {
    ...
    // Act
    val error =
      assertFailsWith<IllegalArgumentException> { HelloMessageCodec.decode(payload = payload) }
