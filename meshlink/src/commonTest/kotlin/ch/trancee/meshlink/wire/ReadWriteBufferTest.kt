...
public class ReadWriteBufferTest {
  ...
  public fun writeByte_supportsZeroInitialCapacityByNormalizingToOneByte(): Unit {
    ...
    assertContentEquals(
      ...
      actual = actualBytes,
      message =
        "WriteBuffer should normalize zero initial capacity and still support single-byte writes",
    )
  }
  ...
  public fun writeByte_supportsDefaultInitialCapacity(): Unit {
    ...
    assertContentEquals(
      ...
      actual = actualBytes,
      message =
        "WriteBuffer should use its default initial capacity when no explicit capacity is provided",
    )
  }
  ...
  public fun readBytes_throwsWhenNegativeLengthIsRequested(): Unit {
    ...
    // Act
    val error = assertFailsWith<IllegalArgumentException> { readBuffer.readBytes(length = -1) }
  ...
  public fun readBytes_throwsWhenRequestedBytesExceedRemainingContent(): Unit {
    ...
    // Act
    val error = assertFailsWith<IllegalArgumentException> { readBuffer.readBytes(length = 3) }
