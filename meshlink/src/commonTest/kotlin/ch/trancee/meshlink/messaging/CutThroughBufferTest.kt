...
public class CutThroughBufferTest {
    ...
    public fun appendVisitedHop_appendsTheRelayHopToChunk0(): Unit {
        ...
        // Act
        val actual = buffer.appendVisitedHop(
            chunk0 = chunk0,
            hopPeerId = hopPeerId,
        )
    ...
    public fun appendVisitedHop_rejectsEmptyInputs(): Unit {
        ...
        // Act
        val emptyChunkError = assertFailsWith<IllegalArgumentException> {
            buffer.appendVisitedHop(chunk0 = byteArrayOf(), hopPeerId = byteArrayOf(0x01))
        }
        val emptyHopError = assertFailsWith<IllegalArgumentException> {
            buffer.appendVisitedHop(chunk0 = byteArrayOf(0x01), hopPeerId = byteArrayOf())
        }

        // Assert
        assertEquals(expected = "CutThroughBuffer chunk0 must not be empty.", actual = emptyChunkError.message)
        assertEquals(expected = "CutThroughBuffer hopPeerId must not be empty.", actual = emptyHopError.message)
    }
}
