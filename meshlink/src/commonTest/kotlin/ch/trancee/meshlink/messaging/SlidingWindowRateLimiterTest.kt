...
public class SlidingWindowRateLimiterTest {
  ...
  public fun invalidInputs_areRejected(): Unit {
    ...
    // Assert
    assertEquals(
      expected = "SlidingWindowRateLimiter windowMillis must be greater than 0.",
      actual = windowError.message,
    )
    assertEquals(
      expected = "SlidingWindowRateLimiter maxMessagesPerWindow must be greater than 0.",
      actual = limitError.message,
    )
    assertEquals(
      expected = "SlidingWindowRateLimiter nowEpochMillis must be greater than or equal to 0.",
