...
public class MessagingFoundationTest {
    ...
    public fun messagingConfig_rejectsNonPositiveWindowLimitTimeoutAndBufferValues(): Unit {
        ...
        val windowError = assertFailsWith<IllegalArgumentException> {
            MessagingConfig(0L, 1, 1L, 1, 0)
        }
        val maxMessagesError = assertFailsWith<IllegalArgumentException> {
            MessagingConfig(1L, 0, 1L, 1, 0)
        }
        val timeoutError = assertFailsWith<IllegalArgumentException> {
            MessagingConfig(1L, 1, 0L, 1, 0)
        }
        val pendingError = assertFailsWith<IllegalArgumentException> {
            MessagingConfig(1L, 1, 1L, 0, 0)
        }

        // Assert
        assertEquals(expected = "MessagingConfig rateLimitWindowMillis must be greater than 0.", actual = windowError.message)
        assertEquals(expected = "MessagingConfig maxMessagesPerWindow must be greater than 0.", actual = maxMessagesError.message)
        assertEquals(expected = "MessagingConfig deliveryTimeoutMillis must be greater than 0.", actual = timeoutError.message)
        assertEquals(expected = "MessagingConfig maxPendingMessages must be greater than 0.", actual = pendingError.message)
    }
    ...
    public fun coreMessagingTypes_retainConstructorValues(): Unit {
        ...
        val inboundMessage = InboundMessage(messageId = messageId, fromPeerId = senderPeerId, payload = payload)
        val delivered: DeliveryOutcome = Delivered(messageId = messageId, peerId = recipientPeerId)
        val failed: DeliveryOutcome = DeliveryFailed(
            messageId = messageId,
            peerId = recipientPeerId,
            reason = DeliveryFailureReason.TIMEOUT,
        )
        val sent: SendResult = SendResult.Sent(messageId = messageId)
        val queued: SendResult = SendResult.Queued(messageId = messageId, reason = QueuedReason.RATE_LIMITED)
        ...
        assertEquals(expected = Delivered(messageId = messageId, peerId = recipientPeerId), actual = delivered)
        assertEquals(
        ...
        assertEquals(expected = SendResult.Queued(messageId = messageId, reason = QueuedReason.RATE_LIMITED), actual = queued)
        assertEquals(expected = SendResult.Rejected(reason = DeliveryFailureReason.BUFFER_PRESSURE), actual = rejected)
    }
    ...
    public fun messageIdKey_rejectsNegativeSequenceNumbers(): Unit {
        ...
        // Assert
        assertEquals(expected = "MessageIdKey sequenceNumber must be greater than or equal to 0.", actual = error.message)
    }
}
