...
public class DiagnosticCodeTest {
  ...
  public fun severities_spanAllFourDiagnosticLevels(): Unit {
    // Arrange
    val expectedLevels: Set<DiagnosticSeverity> =
      setOf(
    ...
    // Act
    val actualLevels: Set<DiagnosticSeverity> =
      DiagnosticCode.entries.map { code -> code.severity }.toSet()
  ...
  public fun errorCodes_arePresentForFailureScenarios(): Unit {
    ...
    // Act
    val actual =
      DiagnosticCode.entries.filter { code -> code.severity == DiagnosticSeverity.ERROR }.toSet()

    // Assert
    assertTrue(
      actual = actual.containsAll(errorCodes),
      message =
        "DiagnosticCode should include explicit ERROR codes for handshake, messaging, transfer, and internal failures",
    )
  }
}
