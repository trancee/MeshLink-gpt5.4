...
public class MeshLinkTest {
  ...
  public fun create_withCustomSinkMapsLifecycleAndMessageFlows(): Unit {
    ...
    val api = MeshLink.create(config = MeshLinkConfig.default(), diagnosticSink = diagnosticSink)

    // Act
    api.start()
    api.send(peerId = PeerIdHex(value = "00112233"), payload = payload)
    ...
    val lastPayload =
      assertIs<DiagnosticPayload.PeerLifecycle>(api.diagnosticEvents.replayCache.last().payload)
  ...
  public fun create_withConfigBuildsSinkFromDiagnosticSettings(): Unit {
    ...
    // Act
    api.send(peerId = PeerIdHex(value = "0011223344556677"), payload = byteArrayOf(0x09))
