...
public class L2capFrameCodecTest {
  ...
  public fun append_allowsAnEmptyChunkWhenBufferedDataExists(): Unit {
    ...
    assertTrue(
      ...
      message =
        "L2capFrameCodec should allow empty chunks while waiting for the remainder of a buffered frame",
    )
  }
  ...
  public fun append_rejectsAnEmptyChunkWhenNothingIsBuffered(): Unit {
    ...
    // Act
    val error = assertFailsWith<IllegalArgumentException> { codec.append(chunk = byteArrayOf()) }
  ...
  public fun encode_rejectsPayloadsLargerThanTheConfiguredMaximum(): Unit {
    ...
    // Act
    val error =
      assertFailsWith<IllegalArgumentException> { codec.encode(payload = byteArrayOf(0x01, 0x02)) }
  ...
  public fun append_rejectsDecodedLengthsOutsideTheConfiguredMaximum(): Unit {
    ...
    // Act
    val error = assertFailsWith<IllegalArgumentException> { codec.append(chunk = encoded) }
  ...
  public fun append_rejectsNegativeDecodedLengths(): Unit {
    // Arrange
    val codec = L2capFrameCodec(maxFrameLengthBytes = 4)
    val encoded = WriteBuffer().apply { writeInt(value = -1) }.toByteArray()
  ...
  public fun init_rejectsNegativeMaximumFrameLengths(): Unit {
    ...
    // Act
    val error =
      assertFailsWith<IllegalArgumentException> { L2capFrameCodec(maxFrameLengthBytes = -1) }
