...
public class VirtualMeshTransportTest {
    ...
    public fun transport_emitsPeerAndMessageDiagnosticsForLifecycleEvents(): Unit {
        ...
        val localPeerId = PeerIdHex(value = "00112233")
        val remotePeerId = PeerIdHex(value = "44556677")
        val localTransport = VirtualMeshTransport(
            localPeerId = localPeerId,
            diagnosticSink = localDiagnosticSink,
        )
        val remoteTransport = VirtualMeshTransport(
        ...
        val localFirstPayload = localDiagnosticSink.diagnosticEvents.replayCache.first().payload as DiagnosticPayload.PeerLifecycle
        val localLastPayload = localDiagnosticSink.diagnosticEvents.replayCache.last().payload as DiagnosticPayload.PeerLifecycle
