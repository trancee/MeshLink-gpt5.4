...
public class MeshHashFilterTest {
  ...
  public fun isDuplicate_rejectsEmptyHashes(): Unit {
    ...
    // Act
    val error =
      assertFailsWith<IllegalArgumentException> { filter.isDuplicate(meshHash = byteArrayOf()) }

    // Assert
    assertEquals(expected = "MeshHashFilter meshHash must not be empty.", actual = error.message)
  }
  ...
  public fun init_rejectsNonPositiveCapacity(): Unit {
    ...
    // Act
    val error = assertFailsWith<IllegalArgumentException> { MeshHashFilter(maxEntries = 0) }
