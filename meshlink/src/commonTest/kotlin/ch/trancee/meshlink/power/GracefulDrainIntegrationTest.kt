...
public class GracefulDrainIntegrationTest {
  ...
  public fun evaluate_keepsInFlightConnectionsAliveUntilTransfersFinish(): Unit {
    ...
    // Act
    val actual =
      manager.evaluate(connections = listOf(inFlightConnection, idleConnection, completeConnection))

    // Assert
    assertEquals(expected = false, actual = actual.drainComplete)
    assertEquals(
      expected = listOf(idleConnection.peerKey, completeConnection.peerKey),
      actual = actual.connectionsToClose,
    )
  }
