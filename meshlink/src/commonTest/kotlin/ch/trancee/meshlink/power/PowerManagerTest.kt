...
public class PowerManagerTest {
  ...
  public fun evaluate_selectsHighNormalAndLowTiersAtTheConfiguredThresholds(): Unit {
    ...
    val powerManager = PowerManager(batteryMonitor = batteryMonitor, config = PowerConfig.default())
  ...
  public fun evaluate_appliesHysteresisToTierTransitions(): Unit {
    ...
    val batteryMonitor = FixedBatteryMonitor(initialBatteryPercent = 80)
    val powerManager = PowerManager(batteryMonitor = batteryMonitor, config = config)
    ...
    val promotedFromLowManager = PowerManager(batteryMonitor = lowBatteryMonitor, config = config)
  ...
  public fun evaluate_returnsProfilesConnectionAdmissionAndShedRecommendations(): Unit {
    ...
    val connections =
      listOf(
        ManagedConnection(
          peerKey = PeerKey(value = "a"),
          transferStatus = TransferStatus.IN_FLIGHT,
          lastActivityEpochMillis = 30L,
        ),
        ManagedConnection(
          peerKey = PeerKey(value = "b"),
          transferStatus = TransferStatus.COMPLETE,
          lastActivityEpochMillis = 10L,
        ),
        ManagedConnection(
          peerKey = PeerKey(value = "c"),
          transferStatus = TransferStatus.IDLE,
          lastActivityEpochMillis = 20L,
        ),
      )
  ...
  public fun policyHelpers_coverAllTierAndSheddingBranches(): Unit {
    ...
    // Act
    val highProfile =
      BleConnectionParameterPolicy.profileFor(tier = PowerTier.HIGH, config = config)
    val normalProfile =
      BleConnectionParameterPolicy.profileFor(tier = PowerTier.NORMAL, config = config)
    ...
    val shedInFlight =
      TieredShedder.shed(
        connections =
          listOf(
            ManagedConnection(
              peerKey = PeerKey(value = "only"),
              transferStatus = TransferStatus.IN_FLIGHT,
              lastActivityEpochMillis = 5L,
            )
          ),
  ...
  public fun helperPoliciesAndTypes_rejectInvalidInputs(): Unit {
    ...
    // Act
    val hysteresisError =
      assertFailsWith<IllegalArgumentException> { PowerModeEngine(hysteresisPercent = -1) }
    val batteryPercentError =
      assertFailsWith<IllegalArgumentException> {
        PowerModeEngine()
          .evaluate(currentTier = null, batteryPercent = 101, config = PowerConfig.default())
      }
    val negativeBatteryPercentError =
      assertFailsWith<IllegalArgumentException> {
        PowerModeEngine()
          .evaluate(currentTier = null, batteryPercent = -1, config = PowerConfig.default())
      }
    val connectionCountError =
      assertFailsWith<IllegalArgumentException> {
        ConnectionLimiter.canAcceptConnection(
    ...
    val peerKeyError = assertFailsWith<IllegalArgumentException> { PeerKey(value = "   ") }
    val connectionError =
      assertFailsWith<IllegalArgumentException> {
        ManagedConnection(
          peerKey = PeerKey(value = "peer"),
          transferStatus = TransferStatus.IDLE,
    ...
    // Assert
    assertEquals(
      expected = "PowerModeEngine hysteresisPercent must be greater than or equal to 0.",
      actual = hysteresisError.message,
    )
    assertEquals(
      expected = "PowerModeEngine batteryPercent must be between 0 and 100.",
      actual = batteryPercentError.message,
    )
    assertEquals(
      expected = "PowerModeEngine batteryPercent must be between 0 and 100.",
      actual = negativeBatteryPercentError.message,
    )
    assertEquals(
      expected = "ConnectionLimiter currentConnectionCount must be greater than or equal to 0.",
      actual = connectionCountError.message,
    )
    assertEquals(
      expected = "TieredShedder targetConnectionCount must be greater than or equal to 0.",
      actual = targetCountError.message,
    )
    assertEquals(expected = "PeerKey value must not be blank.", actual = peerKeyError.message)
    assertEquals(
      expected = "ManagedConnection lastActivityEpochMillis must be greater than or equal to 0.",
      actual = connectionError.message,
    )
  }
}
