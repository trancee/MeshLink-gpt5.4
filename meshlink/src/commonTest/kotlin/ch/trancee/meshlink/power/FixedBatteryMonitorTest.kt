...
public class FixedBatteryMonitorTest {
    ...
    public fun constructionAndUpdates_rejectValuesOutsideTheBatteryRange(): Unit {
        ...
        val initError = assertFailsWith<IllegalArgumentException> {
            FixedBatteryMonitor(initialBatteryPercent = 101)
        }
        val monitor = FixedBatteryMonitor(initialBatteryPercent = 50)
        val updateError = assertFailsWith<IllegalArgumentException> {
            monitor.update(batteryPercent = -1)
        }

        // Assert
        assertEquals(expected = "FixedBatteryMonitor batteryPercent must be between 0 and 100.", actual = initError.message)
        assertEquals(expected = "FixedBatteryMonitor batteryPercent must be between 0 and 100.", actual = updateError.message)
    }
}
