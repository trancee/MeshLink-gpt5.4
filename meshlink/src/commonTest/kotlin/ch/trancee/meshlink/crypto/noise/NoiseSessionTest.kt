...
public class NoiseSessionTest {
  ...
  public fun seal_usesSendCipherState(): Unit {
    // Arrange
    val sendProvider =
      FakeSessionCryptoProvider().apply { nextEncryptResult = byteArrayOf(0x41, 0x42) }
    val receiveProvider = FakeSessionCryptoProvider()
    val sendCipherState =
      CipherState(provider = sendProvider).apply {
        initializeKey(
          key = ByteArray(size = CipherState.KEY_SIZE) { index -> (index + 1).toByte() }
        )
      }
    val receiveCipherState =
      CipherState(provider = receiveProvider).apply {
        initializeKey(
          key = ByteArray(size = CipherState.KEY_SIZE) { index -> (index + 33).toByte() }
        )
      }
    val session =
      NoiseSession(sendCipherState = sendCipherState, receiveCipherState = receiveCipherState)

    // Act
    val actual: ByteArray =
      session.seal(aad = byteArrayOf(0x11), plaintext = byteArrayOf(0x21, 0x22))
  ...
  public fun open_usesReceiveCipherState(): Unit {
    ...
    val receiveProvider =
      FakeSessionCryptoProvider().apply { nextDecryptResult = byteArrayOf(0x61, 0x62) }
    val sendCipherState =
      CipherState(provider = sendProvider).apply {
        initializeKey(
          key = ByteArray(size = CipherState.KEY_SIZE) { index -> (index + 1).toByte() }
    ...
    // Act
    val actual: ByteArray =
      session.open(aad = byteArrayOf(0x31), ciphertext = byteArrayOf(0x71, 0x72))
  ...
  private class FakeSessionCryptoProvider : CryptoProvider {
    ...
    override fun chaCha20Poly1305Encrypt(
      ...
      aad: ByteArray,
      plaintext: ByteArray,
    ): ByteArray = nextEncryptResult.copyOf()
    ...
    ): ByteArray = nextDecryptResult.copyOf()
