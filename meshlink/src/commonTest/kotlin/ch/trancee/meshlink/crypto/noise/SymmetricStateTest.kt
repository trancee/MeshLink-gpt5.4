...
public class SymmetricStateTest {
  ...
  public fun hasCipherKey_returnsFalseBeforeMixKeyRuns(): Unit {
    ...
    assertFalse(
      actual = actual,
      message =
        "SymmetricState should report that no cipher key exists before key material has been mixed",
    )
  }
  ...
  public fun mixHash_updatesHandshakeHashUsingHmacSha256(): Unit {
    ...
    assertContentEquals(
      ...
      actual = actual,
      message =
        "SymmetricState should replace the handshake hash with the HMAC result when mixing hash data",
    )
    assertContentEquals(
  ...
  public fun encryptAndHash_returnsPlaintextWhenCipherKeyIsAbsent(): Unit {
    ...
    assertContentEquals(
      ...
      actual = provider.lastHmacMessage,
      message =
        "SymmetricState should mix the plaintext into the handshake hash when no cipher key is present",
    )
  }
  ...
  public fun encryptAndHash_usesCipherKeyAndCurrentHandshakeHashAsAad(): Unit {
    ...
    assertContentEquals(
      ...
      actual = actualCiphertext,
      message =
        "SymmetricState should return the provider-produced ciphertext once a cipher key exists",
    )
    assertContentEquals(
  ...
  public fun decryptAndHash_returnsCiphertextWhenCipherKeyIsAbsent(): Unit {
    ...
    assertContentEquals(
      ...
      actual = provider.lastHmacMessage,
      message =
        "SymmetricState should mix ciphertext into the handshake hash when decrypting without a cipher key",
    )
  }
  ...
  public fun decryptAndHash_usesCipherKeyAndCurrentHandshakeHashAsAad(): Unit {
    ...
    val actualPlaintext: ByteArray =
      state.decryptAndHash(ciphertext = byteArrayOf(0x71, 0x72, 0x73))
    ...
    assertContentEquals(
      ...
      actual = actualPlaintext,
      message =
        "SymmetricState should return the provider-produced plaintext once a cipher key exists",
    )
    assertContentEquals(
      expected = ByteArray(size = 32) { index -> (index + 33).toByte() },
    ...
    assertContentEquals(
      ...
      message =
        "SymmetricState should authenticate ciphertext against the current handshake hash during decrypt",
    )
    assertContentEquals(
