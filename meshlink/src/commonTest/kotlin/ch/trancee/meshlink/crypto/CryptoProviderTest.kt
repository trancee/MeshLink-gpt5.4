...
public class CryptoProviderTest {
    ...
    public fun primitives_areExposedThroughTheCryptoProviderContract(): Unit {
        ...
        val actualX25519: ByteArray = provider.x25519(privateKey = byteArrayOf(0x01), publicKey = byteArrayOf(0x02))
        val actualSignature: ByteArray = provider.ed25519Sign(privateKey = byteArrayOf(0x03), message = byteArrayOf(0x04))
        val actualVerify: Boolean = provider.ed25519Verify(publicKey = byteArrayOf(0x05), message = byteArrayOf(0x06), signature = byteArrayOf(0x07))
        val actualCiphertext: ByteArray = provider.chaCha20Poly1305Encrypt(
            key = byteArrayOf(0x08),
            nonce = byteArrayOf(0x09),
            aad = byteArrayOf(0x0A),
            plaintext = byteArrayOf(0x0B),
        )
        val actualPlaintext: ByteArray = provider.chaCha20Poly1305Decrypt(
            key = byteArrayOf(0x0C),
            nonce = byteArrayOf(0x0D),
            aad = byteArrayOf(0x0E),
            ciphertext = byteArrayOf(0x0F),
        )
        val actualHkdf: ByteArray = provider.hkdfSha256(
            ikm = byteArrayOf(0x10),
            salt = byteArrayOf(0x11),
            info = byteArrayOf(0x12),
            outputLength = 2,
        )
        val actualHmac: ByteArray = provider.hmacSha256(key = byteArrayOf(0x13), message = byteArrayOf(0x14))
        ...
        assertTrue(actual = actualVerify, message = "CryptoProvider should expose Ed25519 verification through the contract")
        ...
        assertFalse(
            actual = provider.ed25519Verify(publicKey = byteArrayOf(0x05), message = byteArrayOf(0x06), signature = byteArrayOf(0x07)).not(),
    ...
    private class FakeCryptoProvider : CryptoProvider {
        ...
        override fun generateX25519KeyPair(): KeyPair = KeyPair(publicKey = byteArrayOf(0x01), secretKey = byteArrayOf(0x11))

        override fun generateEd25519KeyPair(): KeyPair = KeyPair(publicKey = byteArrayOf(0x02), secretKey = byteArrayOf(0x12))
        ...
        override fun ed25519Verify(publicKey: ByteArray, message: ByteArray, signature: ByteArray): Boolean = nextVerifyResult

        override fun chaCha20Poly1305Encrypt(key: ByteArray, nonce: ByteArray, aad: ByteArray, plaintext: ByteArray): ByteArray = nextCiphertext

        override fun chaCha20Poly1305Decrypt(key: ByteArray, nonce: ByteArray, aad: ByteArray, ciphertext: ByteArray): ByteArray = nextPlaintext

        override fun hkdfSha256(ikm: ByteArray, salt: ByteArray, info: ByteArray, outputLength: Int): ByteArray = nextHkdf
