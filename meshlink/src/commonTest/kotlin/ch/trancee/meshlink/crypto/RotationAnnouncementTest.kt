...
public class RotationAnnouncementTest {
    ...
    public fun create_signsPreviousAndNextPublicKeys(): Unit {
        ...
        val nextPublicKey: ByteArray = ByteArray(size = Identity.PUBLIC_KEY_SIZE) { index -> (index + 65).toByte() }
        val expectedSignature: ByteArray = ByteArray(size = RotationAnnouncement.SIGNATURE_SIZE) { index -> (index + 97).toByte() }
        val provider = FakeRotationCryptoProvider(signatureToReturn = expectedSignature, verifyResult = true)

        // Act
        val actual: RotationAnnouncement = RotationAnnouncement.create(
            provider = provider,
        ...
        assertContentEquals(
            ...
            message = "RotationAnnouncement.create should sign the concatenated previous and next public keys",
        )
    }
    ...
    public fun create_throwsWhenNextPublicKeyLengthIsInvalid(): Unit {
        // Arrange
        val provider = FakeRotationCryptoProvider(
    ...
    public fun verify_returnsTrueWhenProviderAcceptsSignature(): Unit {
        // Arrange
        val announcement = RotationAnnouncement(
            previousPublicKey = ByteArray(size = Identity.PUBLIC_KEY_SIZE) { index -> (index + 1).toByte() },
            nextPublicKey = ByteArray(size = Identity.PUBLIC_KEY_SIZE) { index -> (index + 33).toByte() },
            signature = ByteArray(size = RotationAnnouncement.SIGNATURE_SIZE) { index -> (index + 65).toByte() },
        )
        val provider = FakeRotationCryptoProvider(
            signatureToReturn = ByteArray(size = RotationAnnouncement.SIGNATURE_SIZE),
            verifyResult = true,
        ...
        assertTrue(
            actual = actual,
            message = "RotationAnnouncement.verify should return true when the provider accepts the signature",
        )
        assertContentEquals(
        ...
        assertContentEquals(
            ...
            message = "RotationAnnouncement.verify should verify the concatenated previous and next public keys",
        )
        assertContentEquals(
    ...
    public fun verify_returnsFalseWhenProviderRejectsSignature(): Unit {
        ...
        assertFalse(
            actual = actual,
            message = "RotationAnnouncement.verify should return false when the provider rejects the signature",
        )
    }
    ...
    public fun verify_throwsWhenSignatureLengthIsInvalid(): Unit {
        ...
        // Act
        val error = assertFailsWith<IllegalArgumentException> {
            announcement.verify(provider = provider)
        }
    ...
    private class FakeRotationCryptoProvider(
        ...
        override fun chaCha20Poly1305Encrypt(key: ByteArray, nonce: ByteArray, aad: ByteArray, plaintext: ByteArray): ByteArray = unsupported()

        override fun chaCha20Poly1305Decrypt(key: ByteArray, nonce: ByteArray, aad: ByteArray, ciphertext: ByteArray): ByteArray = unsupported()
