...
public class UnsupportedCryptoProviderTest {
    ...
    public fun allPrimitiveOperations_throwHelpfulPlaceholderMessage(): Unit {
        ...
        val hkdfError = assertFailsWith<UnsupportedOperationException> {
            provider.hkdfSha256(
        ...
        val hmacError = assertFailsWith<UnsupportedOperationException> {
            provider.hmacSha256(key = byteArrayOf(0x13), message = byteArrayOf(0x14))
        }

        // Assert
        val expectedMessage = "CryptoProviderFactory has not been wired to a platform crypto backend yet."
