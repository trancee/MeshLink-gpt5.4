...
public class DhCacheTest {
    ...
    public fun getOrCompute_returnsCachedResultForRepeatedKeyPair(): Unit {
        ...
        // Act
        val first: ByteArray = cache.getOrCompute(
            privateKey = byteArrayOf(0x01),
            publicKey = byteArrayOf(0x02),
        ) {
            computeCalls += 1
            expectedSharedSecret
        }
        val second: ByteArray = cache.getOrCompute(
            privateKey = byteArrayOf(0x01),
            publicKey = byteArrayOf(0x02),
        ) {
            computeCalls += 1
    ...
    public fun getOrCompute_returnsDefensiveCopiesOfCachedValues(): Unit {
        // Arrange
        val cache = DhCache(maxEntries = 1)
        val initialSecret: ByteArray = cache.getOrCompute(
            privateKey = byteArrayOf(0x05),
            publicKey = byteArrayOf(0x06),
        ) {
        ...
        // Act
        val actual: ByteArray = cache.getOrCompute(
            privateKey = byteArrayOf(0x05),
            publicKey = byteArrayOf(0x06),
        ) {
