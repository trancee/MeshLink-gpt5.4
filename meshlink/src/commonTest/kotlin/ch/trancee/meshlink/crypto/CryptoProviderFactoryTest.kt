...
public class CryptoProviderFactoryTest {
  ...
  public fun create_returnsCryptoProviderImplementation(): Unit {
    ...
    assertIs<CryptoProvider>(
      value = actual,
      message =
        "CryptoProviderFactory should return a CryptoProvider implementation for the current platform",
    )
  }
  ...
  public fun x25519_returnsSharedSecretOrThrowsHelpfulPlaceholderError(): Unit {
    ...
    if (firstKeyPair.isSuccess) {
      ...
      assertContentEquals(
        ...
        message =
          "Platform crypto providers should derive the same X25519 shared secret for both peers",
      )
    } else {
  ...
  public fun ed25519SignVerify_returnsTrueOrThrowsHelpfulPlaceholderError(): Unit {
    ...
    if (keyPairResult.isSuccess) {
      ...
      assertEquals(
        ...
        actual = operation.exceptionOrNull()?.message,
        message =
          "Placeholder platforms should surface the missing Ed25519 signing operation explicitly",
      )
    }
  }
  ...
  public fun chaCha20Poly1305_roundTripsOrThrowsHelpfulPlaceholderError(): Unit {
    ...
    if (result.isSuccess) {
      assertContentEquals(
        ...
        message =
          "Platform crypto providers should round-trip plaintext through ChaCha20-Poly1305 when available",
      )
    } else {
      assertEquals(
        expected = "CryptoProviderFactory has not been wired to a platform crypto backend yet.",
        actual = result.exceptionOrNull()?.message,
        message =
          "Placeholder platforms should surface the missing ChaCha20-Poly1305 backend explicitly",
      )
    }
  }
