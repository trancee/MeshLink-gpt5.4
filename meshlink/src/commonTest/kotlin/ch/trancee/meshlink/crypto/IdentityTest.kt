...
public class IdentityTest {
  ...
  public fun generate_usesProviderKeyPairAndDerivesKeyHash(): Unit {
    ...
    val expectedSecretKey: ByteArray =
      ByteArray(size = Identity.SECRET_KEY_SIZE) { index -> (index + 33).toByte() }
    val expectedKeyHash: ByteArray = byteArrayOf(0x61, 0x62, 0x63)
    val provider =
      FakeCryptoProvider(
        generatedEd25519KeyPair =
          KeyPair(publicKey = expectedPublicKey, secretKey = expectedSecretKey),
  ...
  public fun generate_usesDefaultProviderWhenAvailableOrThrowsHelpfulPlaceholderError(): Unit {
    ...
    if (result.isSuccess) {
      ...
      assertEquals(
        ...
        message =
          "Identity.generate should surface the missing platform crypto backend when the current platform has no provider",
      )
    }
  }
  ...
  public fun fromKeyPair_throwsWhenPublicKeyLengthIsInvalid(): Unit {
    ...
    val keyPair =
      KeyPair(publicKey = byteArrayOf(0x01), secretKey = ByteArray(size = Identity.SECRET_KEY_SIZE))
  ...
  public fun fromKeyPair_throwsWhenSecretKeyLengthIsInvalid(): Unit {
    ...
    val keyPair =
      KeyPair(publicKey = ByteArray(size = Identity.PUBLIC_KEY_SIZE), secretKey = byteArrayOf(0x01))
