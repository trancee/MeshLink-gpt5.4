...
public class TrustStoreTest {
  ...
  public fun evaluate_pinsNewPeerInTofuMode(): Unit {
    ...
    val decision: TrustDecision =
      store.evaluate(peerId = peerId, presentedPublicKey = publicKey, mode = TrustMode.TOFU)
  ...
  public fun evaluate_acceptsPinnedPeerWhenKeyMatches(): Unit {
    ...
    // Act
    val decision: TrustDecision =
      store.evaluate(peerId = peerId, presentedPublicKey = publicKey, mode = TrustMode.STRICT)
  ...
  public fun evaluate_rejectsUnknownPeerInStrictMode(): Unit {
    ...
    // Assert
    val rejected: TrustDecision.Rejected =
      assertIs<TrustDecision.Rejected>(
        value = decision,
  ...
  public fun evaluate_rejectsMismatchedPinnedKeyInTofuModeAfterInitialPin(): Unit {
    ...
    val rejected: TrustDecision.Rejected =
      assertIs<TrustDecision.Rejected>(
        value = decision,
        message =
          "TrustStore should reject mismatched keys in TOFU mode after the first contact has been pinned",
      )
    assertEquals(
      expected = "Presented key does not match pinned key.",
      actual = rejected.reason,
  ...
  public fun evaluate_requestsPromptForUnknownPeerInPromptMode(): Unit {
    ...
    // Assert
    val promptRequired: TrustDecision.PromptRequired =
      assertIs<TrustDecision.PromptRequired>(
        value = decision,
  ...
  public fun evaluate_requestsPromptForMismatchedPinnedKeyInPromptMode(): Unit {
    ...
    val promptRequired: TrustDecision.PromptRequired =
      assertIs<TrustDecision.PromptRequired>(
        value = decision,
        message =
          "TrustStore should request a prompt when a pinned peer presents a different key in PROMPT mode",
      )
    assertContentEquals(
  ...
  public fun pinnedKey_returnsDefensiveCopy(): Unit {
    ...
    assertContentEquals(
      ...
      actual = secondRead,
      message =
        "TrustStore should protect pinned keys from caller mutation by returning defensive copies",
    )
  }
}
