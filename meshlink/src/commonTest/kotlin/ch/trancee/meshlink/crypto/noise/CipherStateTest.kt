...
public class CipherStateTest {
    ...
    public fun initializeKey_setsKeyAndResetsNonce(): Unit {
        ...
        // Act
        state.initializeKey(key = ByteArray(size = CipherState.KEY_SIZE) { index -> (index + 1).toByte() })
    ...
    public fun initializeKey_throwsWhenKeyLengthIsInvalid(): Unit {
        ...
        val error = assertFailsWith<IllegalArgumentException> {
            state.initializeKey(key = byteArrayOf(0x01))
        }
    ...
    public fun encryptWithAd_usesCurrentNonceAndAdvancesCounter(): Unit {
        ...
        // Act
        val actual: ByteArray = state.encryptWithAd(aad = byteArrayOf(0x41), plaintext = byteArrayOf(0x51))
        ...
        assertContentEquals(
            expected = byteArrayOf(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00),
    ...
    public fun decryptWithAd_usesCurrentNonceAndAdvancesCounter(): Unit {
        ...
        // Act
        val actual: ByteArray = state.decryptWithAd(aad = byteArrayOf(0x71), ciphertext = byteArrayOf(0x72, 0x73))
