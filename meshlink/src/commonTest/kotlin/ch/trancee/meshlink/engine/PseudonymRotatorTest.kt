...
public class PseudonymRotatorTest {
  ...
  public fun epochFor_changesAtConfiguredBoundaries(): Unit {
    // Arrange
    val rotator =
      PseudonymRotator(cryptoProvider = FakeCryptoProvider(), epochDurationMillis = 100L)
  ...
  public fun staggerMillis_isDeterministicAndBoundedPerNode(): Unit {
    // Arrange
    val rotator = PseudonymRotator(cryptoProvider = FakeCryptoProvider(), maxStaggerMillis = 10L)
  ...
  public fun pseudonymForEpoch_returnsTheConfiguredAdvertisementLength(): Unit {
    // Arrange
    val rotator = PseudonymRotator(cryptoProvider = FakeCryptoProvider(), pseudonymLengthBytes = 12)

    // Act
    val actual = rotator.pseudonymForEpoch(identityKey = byteArrayOf(0x01), epoch = 1L)
  ...
  public fun init_rejectsInvalidEpochDurations(): Unit {
    ...
    val error =
      assertFailsWith<IllegalArgumentException> {
        PseudonymRotator(cryptoProvider = FakeCryptoProvider(), epochDurationMillis = 0L)
      }
  ...
  public fun init_rejectsInvalidPseudonymLengths(): Unit {
    ...
    val tooSmallError =
      assertFailsWith<IllegalArgumentException> {
        PseudonymRotator(cryptoProvider = FakeCryptoProvider(), pseudonymLengthBytes = 0)
      }
    val tooLargeError =
      assertFailsWith<IllegalArgumentException> {
        PseudonymRotator(cryptoProvider = FakeCryptoProvider(), pseudonymLengthBytes = 33)
      }
  ...
  public fun init_rejectsNegativeMaxStaggerValues(): Unit {
    ...
    val error =
      assertFailsWith<IllegalArgumentException> {
        PseudonymRotator(cryptoProvider = FakeCryptoProvider(), maxStaggerMillis = -1L)
      }
  ...
  public fun epochFor_rejectsNegativeTimestamps(): Unit {
    ...
    // Act
    val error =
      assertFailsWith<IllegalArgumentException> { rotator.epochFor(timestampMillis = -1L) }
  ...
  public fun staggerMillis_rejectsEmptyNodeIds(): Unit {
    ...
    // Act
    val error =
      assertFailsWith<IllegalArgumentException> { rotator.staggerMillis(nodeId = byteArrayOf()) }

    // Assert
    assertEquals(expected = "PseudonymRotator nodeId must not be empty.", actual = error.message)
  }

  @Test
  public fun staggerMillis_returnsZeroWhenNoStaggerIsConfigured(): Unit {
    // Arrange
    val rotator = PseudonymRotator(cryptoProvider = FakeCryptoProvider(), maxStaggerMillis = 0L)
  ...
  public fun pseudonymForEpoch_rejectsInvalidInputs(): Unit {
    ...
    // Assert
    assertEquals(
      expected = "PseudonymRotator identityKey must not be empty.",
      actual = emptyKeyError.message,
    )
    assertEquals(
      expected = "PseudonymRotator epoch must be greater than or equal to 0.",
      actual = negativeEpochError.message,
    )
  }
