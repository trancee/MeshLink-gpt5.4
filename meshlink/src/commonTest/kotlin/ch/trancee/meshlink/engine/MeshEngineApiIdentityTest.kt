...
public class MeshEngineApiIdentityTest {
    ...
    public fun verifyPseudonym_acceptsCurrentWindowCandidatesAndRejectsOlderOnes(): Unit {
        ...
        val identityKey = byteArrayOf(0x11, 0x22)
        val validCandidate = engine.pseudonymAt(
            identityKey = identityKey,
            timestampMillis = 1_000L,
        )
        val invalidCandidate = engine.pseudonymRotator.pseudonymForEpoch(
            identityKey = identityKey,
            epoch = 5L,
        )

        // Act
        val validActual = engine.verifyPseudonym(
            candidate = validCandidate,
            identityKey = identityKey,
            timestampMillis = 1_000L,
        )
        val invalidActual = engine.verifyPseudonym(
