...
public class NoiseHandshakeManagerDiagnosticTest {
  @Test
  public fun managers_completeTheThreeMessageHandshakeAndEmitStartedAndSucceededDiagnostics():
    Unit {
    // Arrange
    ...
    val initiatorPayload =
      assertIs<DiagnosticPayload.PeerLifecycle>(
        initiatorSink.diagnosticEvents.replayCache.last().payload
      )
    val responderPayload =
      assertIs<DiagnosticPayload.PeerLifecycle>(
        responderSink.diagnosticEvents.replayCache.last().payload
      )
  ...
  public fun receiveHandshakeMessage_createsResponderStateLazilyOnFirstInboundMessage(): Unit {
    ...
    val payload =
      assertIs<DiagnosticPayload.PeerLifecycle>(sink.diagnosticEvents.replayCache.single().payload)
  ...
  public fun createOutboundMessage_rejectsPeersWithoutAnActiveHandshake(): Unit {
    ...
    val error =
      assertFailsWith<IllegalArgumentException> {
        manager.createOutboundMessage(peerId = peerId, payload = byteArrayOf(0x01))
      }
  ...
  public fun receiveHandshakeMessage_emitsFailureDiagnosticsAndClearsStateOnProtocolErrors(): Unit {
    ...
    val error =
      assertFailsWith<IllegalArgumentException> {
        manager.receiveHandshakeMessage(
          peerId = peerId,
          role = HandshakeRole.RESPONDER,
          message = HandshakeMessage(round = HandshakeRound.TWO, payload = byteArrayOf(0x55)),
        )
      }
    ...
    val payload =
      assertIs<DiagnosticPayload.HandshakeFailure>(sink.diagnosticEvents.replayCache.last().payload)
  ...
  public fun createOutboundMessage_emitsFailureDiagnosticsAndClearsStateOnRoleViolations(): Unit {
    ...
    val error =
      assertFailsWith<IllegalStateException> {
        manager.createOutboundMessage(peerId = peerId, payload = byteArrayOf(0x24))
      }
