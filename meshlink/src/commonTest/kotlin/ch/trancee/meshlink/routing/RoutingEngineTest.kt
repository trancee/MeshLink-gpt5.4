...
public class RoutingEngineTest {
  ...
  public fun processUpdate_rejectsInfeasibleRoutesFromTheCoordinator(): Unit {
    ...
    assertEquals(expected = false, actual = actual)
    assertEquals(
      expected = PeerIdHex(value = "44556677"),
      actual = engine.nextHopFor(destinationPeerId = destinationPeerId),
    )
  }
  ...
  public fun routingUpdate_rejectsInvalidMetricSequenceNumberAndExpiryValues(): Unit {
    // Arrange
    // Act
    val metricError = assertFailsWith<IllegalArgumentException> { routingUpdate(metric = -1) }
    val sequenceNumberError =
      assertFailsWith<IllegalArgumentException> { routingUpdate(sequenceNumber = -1) }
    val expiryError =
      assertFailsWith<IllegalArgumentException> { routingUpdate(expiresAtEpochMillis = -1L) }

    // Assert
    assertEquals(
      expected = "RoutingUpdate metric must be greater than or equal to 0.",
      actual = metricError.message,
    )
    assertEquals(
      expected = "RoutingUpdate sequenceNumber must be greater than or equal to 0.",
      actual = sequenceNumberError.message,
    )
    assertEquals(
      expected = "RoutingUpdate expiresAtEpochMillis must be greater than or equal to 0.",
