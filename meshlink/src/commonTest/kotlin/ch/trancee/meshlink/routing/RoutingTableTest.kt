...
public class RoutingTableTest {
    ...
    public fun bestRoute_prefersTheLowestMetricRoute(): Unit {
        ...
        val lowerMetricRoute = routeEntry(
            destinationPeerId = destinationPeerId,
            nextHopPeerId = PeerIdHex(value = "44556677"),
            metric = 1,
        )
        val higherMetricRoute = routeEntry(
            destinationPeerId = destinationPeerId,
    ...
    public fun routesFor_sortsTiesBySequenceNumberAndThenNextHopId(): Unit {
        // Arrange
        val destinationPeerId = PeerIdHex(value = "00112233")
        val olderRoute = routeEntry(
            destinationPeerId = destinationPeerId,
            nextHopPeerId = PeerIdHex(value = "ccddeeff"),
            metric = 2,
            sequenceNumber = 4,
        )
        val newerRoute = routeEntry(
            destinationPeerId = destinationPeerId,
            nextHopPeerId = PeerIdHex(value = "44556677"),
            metric = 2,
            sequenceNumber = 7,
        )
        val tieBreakRoute = routeEntry(
            destinationPeerId = destinationPeerId,
            nextHopPeerId = PeerIdHex(value = "8899aabb"),
            metric = 2,
            sequenceNumber = 7,
        ...
        // Assert
        assertEquals(
            expected = listOf(newerRoute, tieBreakRoute, olderRoute),
            actual = actual,
        )
    }
    ...
    public fun withdraw_removesOnlyTheSpecifiedNextHopAndPreservesAlternatives(): Unit {
        // Arrange
        val destinationPeerId = PeerIdHex(value = "00112233")
        val withdrawnRoute = routeEntry(
            destinationPeerId = destinationPeerId,
            nextHopPeerId = PeerIdHex(value = "44556677"),
            metric = 1,
        )
        val fallbackRoute = routeEntry(
            destinationPeerId = destinationPeerId,
            nextHopPeerId = PeerIdHex(value = "8899aabb"),
            metric = 2,
        )
        ...
        val actualRoutes: List<RouteEntry> = routingTable.routesFor(destinationPeerId = destinationPeerId)
    ...
    public fun withdraw_removesDestinationsThatNoLongerHaveRoutes(): Unit {
        ...
        // Assert
        assertEquals(expected = emptyList(), actual = routingTable.routesFor(destinationPeerId = route.destinationPeerId))
    ...
    public fun withdraw_ignoresMissingRoutes(): Unit {
        ...
        // Assert
        assertEquals(expected = listOf(route), actual = routingTable.routesFor(destinationPeerId = destinationPeerId))
    }
    ...
    public fun destinations_returnsAllKnownDestinations(): Unit {
        ...
        val routingTable = RoutingTable()
        routingTable.install(route = routeEntry(destinationPeerId = firstDestination, nextHopPeerId = PeerIdHex(value = "8899aabb")))
        routingTable.install(route = routeEntry(destinationPeerId = secondDestination, nextHopPeerId = PeerIdHex(value = "ccddeeff")))
    ...
    public fun routeEntry_rejectsInvalidMetricsSequenceNumbersAndExpiryValues(): Unit {
        ...
        val sequenceNumberError = assertFailsWith<IllegalArgumentException> {
            routeEntry(sequenceNumber = -1)
        }
        val expiryError = assertFailsWith<IllegalArgumentException> {
            routeEntry(expiresAtEpochMillis = -1L)
        }

        // Assert
        assertEquals(expected = "RouteEntry metric must be greater than or equal to 0.", actual = metricError.message)
        assertEquals(expected = "RouteEntry sequenceNumber must be greater than or equal to 0.", actual = sequenceNumberError.message)
        assertEquals(expected = "RouteEntry expiresAtEpochMillis must be greater than or equal to 0.", actual = expiryError.message)
    }
