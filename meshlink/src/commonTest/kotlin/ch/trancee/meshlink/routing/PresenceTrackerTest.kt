...
public class PresenceTrackerTest {
    ...
    public fun observe_andSweep_rejectNegativeTimestamps(): Unit {
        ...
        // Act
        val observeError = assertFailsWith<IllegalArgumentException> {
            tracker.observe(peerId = peerId, nowEpochMillis = -1L)
        }
        val sweepError = assertFailsWith<IllegalArgumentException> {
            tracker.sweep(nowEpochMillis = -1L)
        }

        // Assert
        assertEquals(expected = "PresenceTracker nowEpochMillis must be greater than or equal to 0.", actual = observeError.message)
        assertEquals(expected = "PresenceTracker nowEpochMillis must be greater than or equal to 0.", actual = sweepError.message)
    }
    ...
    public fun init_rejectsNonPositiveTimeouts(): Unit {
        ...
        val error = assertFailsWith<IllegalArgumentException> {
            PresenceTracker(timeoutMillis = 0L)
        }
