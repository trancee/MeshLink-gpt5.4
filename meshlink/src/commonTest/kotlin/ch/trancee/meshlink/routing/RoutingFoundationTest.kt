...
public class RoutingFoundationTest {
    ...
    public fun routingConfig_rejectsInvalidValues(): Unit {
        // Arrange
        // Act
        val routeExpiryError = assertFailsWith<IllegalArgumentException> {
            RoutingConfig(routeExpiryMillis = 0L, peerTimeoutMillis = 1L, hopLimit = 1)
        }
        val peerTimeoutError = assertFailsWith<IllegalArgumentException> {
            RoutingConfig(routeExpiryMillis = 1L, peerTimeoutMillis = 0L, hopLimit = 1)
        }
        val hopLimitError = assertFailsWith<IllegalArgumentException> {
            RoutingConfig(routeExpiryMillis = 1L, peerTimeoutMillis = 1L, hopLimit = 0)
        }

        // Assert
        assertEquals(expected = "RoutingConfig routeExpiryMillis must be greater than 0.", actual = routeExpiryError.message)
        assertEquals(expected = "RoutingConfig peerTimeoutMillis must be greater than 0.", actual = peerTimeoutError.message)
        assertEquals(expected = "RoutingConfig hopLimit must be greater than 0.", actual = hopLimitError.message)
    }
    ...
    public fun peerInfo_rejectsInvalidMetricAndTimestampValues(): Unit {
        ...
        // Assert
        assertEquals(expected = "PeerInfo metric must be greater than or equal to 0.", actual = metricError.message)
        assertEquals(expected = "PeerInfo lastSeenEpochMillis must be greater than or equal to 0.", actual = timestampError.message)
    }
    ...
    public fun outboundFrame_rejectsNegativeHopCounts(): Unit {
        ...
        // Assert
        assertEquals(expected = "OutboundFrame hopCount must be greater than or equal to 0.", actual = error.message)
    }
}
