...
public class DedupSetTest {
    ...
    public fun isDuplicate_rejectsInvalidInputs(): Unit {
        ...
        assertEquals(expected = "DedupSet nowEpochMillis must be greater than or equal to 0.", actual = negativeTimestampError.message)
    }
    ...
    public fun init_rejectsNonPositiveCapacityAndExpiryValues(): Unit {
        // Arrange
        // Act
        val maxEntriesError = assertFailsWith<IllegalArgumentException> {
            DedupSet(maxEntries = 0)
        }
        val expiryError = assertFailsWith<IllegalArgumentException> {
            DedupSet(expiryMillis = 0L)
        }

        // Assert
        assertEquals(expected = "DedupSet maxEntries must be greater than 0.", actual = maxEntriesError.message)
        assertEquals(expected = "DedupSet expiryMillis must be greater than 0.", actual = expiryError.message)
    }
}
