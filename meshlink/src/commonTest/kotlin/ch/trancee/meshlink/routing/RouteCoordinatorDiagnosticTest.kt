...
public class RouteCoordinatorDiagnosticTest {
    ...
    public fun requestSequenceNumber_andWithdraw_emitRouteRemovedDiagnostics(): Unit {
        ...
        // Act
        val requestedSequenceNumber = coordinator.requestSequenceNumber(destinationPeerId = destinationPeerId)
        ...
        val requestPayload = assertIs<DiagnosticPayload.RoutingChange>(diagnosticSink.diagnosticEvents.replayCache[1].payload)
        val withdrawPayload = assertIs<DiagnosticPayload.RoutingChange>(diagnosticSink.diagnosticEvents.replayCache[2].payload)

        // Assert
        assertEquals(expected = 5, actual = requestedSequenceNumber)
        assertEquals(
            expected = listOf(DiagnosticCode.ROUTE_ADDED, DiagnosticCode.ROUTE_REMOVED, DiagnosticCode.ROUTE_REMOVED),
            actual = actualCodes,
        )
