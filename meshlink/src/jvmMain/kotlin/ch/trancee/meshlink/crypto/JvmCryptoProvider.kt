...
public class JvmCryptoProvider : CryptoProvider {
    ...
    override fun generateEd25519KeyPair(): KeyPair {
        ...
        return KeyPair(
            publicKey = publicKey,
            secretKey = privateSeed + publicKey,
        )
    }
    ...
    override fun ed25519Sign(
        ...
        val privateSeed: ByteArray = privateKey.copyOfRange(
            fromIndex = 0,
            toIndex = Identity.PUBLIC_KEY_SIZE,
        )
        val keyFactory = KeyFactory.getInstance("Ed25519")
        val privateKeySpec = EdECPrivateKeySpec(
    ...
    override fun ed25519Verify(
        ...
        val keyFactory = KeyFactory.getInstance("Ed25519")
        val publicKeySpec = EdECPublicKeySpec(
            NamedParameterSpec.ED25519,
            point,
        )
    ...
    private fun encodeEd25519PublicKey(key: EdECPublicKey): ByteArray {
        ...
        val encoded: ByteArray = toLittleEndian(
            value = point.y,
            size = Identity.PUBLIC_KEY_SIZE,
        )
        encoded[encoded.lastIndex] = (encoded.last().toInt() or (point.isXOdd.compareTo(false) shl 7)).toByte()
        return encoded
    }

    private fun decodeEd25519Point(publicKey: ByteArray): EdECPoint {
        val yBytes: ByteArray = publicKey.copyOf().also { bytes ->
            bytes[bytes.lastIndex] = (bytes.last().toInt() and 0x7F).toByte()
        }
        return EdECPoint(
            (publicKey.last().toInt() and 0x80) != 0,
            fromLittleEndian(bytes = yBytes),
        )
    }
